...
def hist_resp_builder(reco_lead,reco_sublead,part_lead,part_sublead,do_response = False):
...
def process(reco_tree_path, part_tree_path, list_recovars, dict_partvars, do_response = False):
    ...
    reco_pt_emu = np.sort(np.hstack((electron_pt[mask_emu], muon_pt[mask_emu])), axis=-1)
    ...
    return hist_resp_builder(reco_pT_lep1, reco_pT_lep2, particle_pT_lep1, particle_pT_lep2,do_response)
...
if __name__ == '__main__':
    ...
    list_recovars = ["Electron_size", "Muon_size", "Electron.PT", "Muon.PT","Electron.Eta"] 
